...
export default function AccountScreen() {
  ...
  if (!ready) return <LoadingState label="Abrindo sua conta…" />;
    ...
    return (
      <Screen contentStyle={styles.guestContent}>
        ...
        <View style={styles.guestHero}>
          ...
          <Text style={styles.guestTitle}>Sua noite, do seu jeito.</Text>
          <Text style={styles.guestText}>Crie uma conta normal usando seu nome, e-mail e senha. O Google fica apenas como uma alternativa.</Text>
        </View>
  ...
  async function submitReview() {
    ...
    try {
      ...
      Alert.alert('Publicação salva', 'Ela já aparece no seu perfil. Se você estiver offline, será enviada quando a internet voltar.');
    } catch (error) {
  ...
  async function choosePhoto(source: 'camera' | 'library') {
    try {
      ...
      Alert.alert('Não foi possível usar a foto', error instanceof Error ? error.message : 'Tente escolher outra imagem.');
    }
  }
  ...
  return (
    <Screen>
      <View style={styles.headingRow}>
        <View style={styles.headingCopy}><Text style={styles.eyebrow}>{profile?.role === 'owner' ? 'ESTABELECIMENTO' : 'MINHA CONTA'}</Text><Text style={styles.title}>Olá, {profile?.fullName?.split(' ')[0] || 'NightGuide'}</Text></View>
        <LanguageSwitch />
      </View>
      <Text style={styles.email}>{user.email}</Text>

      <View style={styles.metrics}>
        <Metric icon={Heart} label="Favoritos" value={favoriteIds.length} />
        <Metric icon={Ticket} label="Ingressos" value={tickets.length} />
        <Metric icon={MessageSquare} label="Reviews" value={reviews.length} />
      </View>

      {profile?.role === 'owner' ? (
        <Button label="Gerenciar estabelecimento" icon={Store} onPress={() => router.push('/owner')} style={styles.ownerButton} />
      ) : null}

      <Section title="Eventos salvos">
        ...
        )) : <Text style={styles.emptyText}>Salve eventos na tela Descobrir para vê-los aqui.</Text>}
      </Section>

      <Section title={editingReviewId ? 'Editar publicação' : 'Criar publicação'}>
        ...
        <FormField label="Descrição" placeholder="Conte como foi sua experiência nesse local" value={comment} onChangeText={setComment} multiline maxLength={500} />
        <Text style={styles.label}>Foto (opcional)</Text>
        <View style={styles.mediaActions}>
          <Button label="Galeria" icon={ImageIcon} variant="secondary" onPress={() => void choosePhoto('library')} style={styles.mediaButton} />
          <Button label="Câmera" icon={Camera} variant="secondary" onPress={() => void choosePhoto('camera')} style={styles.mediaButton} />
        </View>
        {photo?.uri || existingPhotoUrl ? (
          <View style={styles.previewWrap}>
            <Image source={{ uri: photo?.uri || existingPhotoUrl }} resizeMode="cover" style={styles.preview} />
            <Pressable accessibilityLabel="Remover foto" onPress={removePhoto} style={styles.removePhoto}><Trash2 size={18} color={colors.text} /></Pressable>
          </View>
        ) : null}
        <Button label={editingReviewId ? 'Salvar alterações' : 'Publicar'} icon={MessageSquare} onPress={() => void submitReview()} loading={sending} disabled={!comment.trim()} style={styles.submit} />
        {editingReviewId ? <Button label="Cancelar edição" icon={X} variant="ghost" onPress={cancelEditing} /> : null}
      </Section>
          ...
          {reviews.map((review) => (
            <View key={review.id} style={styles.post}>
              <View style={styles.postHeader}>
                ...
                <Pressable accessibilityLabel="Editar publicação" onPress={() => startEditing(review)} style={styles.editPost}>
              ...
              <View style={styles.postFooter}>
                ...
                <Text style={styles.postDate}>{new Date(review.createdAt).toLocaleDateString('pt-BR')}</Text>
              </View>
            </View>
          ))}
        </Section>
      ...
      {profile?.role === 'owner' ? <Button label="Abrir scanner de ingressos" icon={QrCode} variant="secondary" onPress={() => router.push('/scanner')} /> : null}
      <Button label="Sair da conta" icon={LogOut} variant="danger" onPress={() => void logout()} style={styles.logout} />
    </Screen>
  );
}
