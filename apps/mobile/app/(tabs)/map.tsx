...
export default function MapScreen() {
  ...
  async function route() {
    try { await Linking.openURL(`https://www.google.com/maps/dir/?api=1&destination=${lat},${lon}`); }
  ...
  return <Screen>
    ...
    {selected ? <>
      ...
      <Text style={styles.meta}>{selected.category} · {selected.address}</Text>
      <Text style={[styles.text, { marginVertical: 14 }]}>{selected.vibe}</Text>
      <Button label={pt ? 'Ver perfil e avaliações' : 'Profile and reviews'} onPress={() => router.push({ pathname: '/venue/[id]', params: { id: selected.id } })} />
      <Button label={pt ? 'Traçar rota' : 'Directions'} variant="secondary" onPress={() => void route()} style={{ marginTop: 10 }} />
