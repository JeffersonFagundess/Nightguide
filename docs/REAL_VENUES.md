...
Demais capas: imagens ilustrativas do Unsplash por categoria, com aviso visível. URLs em apps/mobile/src/data/venue-photos.ts. Licença: https://unsplash.com/license. Não são fotografias dos estabelecimentos e não indicam parceria.

## Versão Android 1.1.0

O mapa usa OpenStreetMap dentro de WebView, sem exigir chave Google Maps no APK. Seleção de local muda o marcador; Traçar rota abre o serviço externo. As ruas exigem internet. Catálogo e textos já carregados ficam disponíveis offline; fotos remotas dependem do cache do dispositivo e não têm disponibilidade offline garantida.
